...
class StaffDirectoryPlugin_Importer
{
	...
	public static function output_form()
	{
		...
		/*
		echo "<h2>Import From CSV File</h2>";
		...
		echo '<div class="gp_upload_file_wrapper">';
			...
			$new_open_form_tag = '<div data-gp-ajax-form="1" ';
			$new_close_form_tag = '<input type="hidden" name="_company_dir_do_import" value="_company_dir_do_import" /></div>';
			
			$import_form_html = ob_get_contents();
			$import_form_html = str_replace('<form ', $new_open_form_tag, $import_form_html);
			$import_form_html = str_replace('</form>', $new_close_form_tag, $import_form_html);
			ob_end_clean();					
